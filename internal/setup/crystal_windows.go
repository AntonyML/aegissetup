...
// e inyecta de forma transparente el usuario y la contraseña SQL.
// Esto permite que reportes con Trusted_Connection=No abran y visualicen datos
// correctamente sin depender de un DSN con PWD ni exponer credenciales en el repositorio.
func PatchCrystalODBCBridge(appDir, user, pass string, out func(string)) error {
	if appDir == "" {
	...
	var dllBytes []byte
	var err error
	if crystalFS := assets.Crystal(); crystalFS != nil {
		dllBytes, err = fs.ReadFile(crystalFS, "p2sodbc.dll")
	}
	if err != nil || len(dllBytes) == 0 {
		srcDll := filepath.Join(SysWOW64, "p2sodbc.dll")
		dllBytes, err = os.ReadFile(srcDll)
		if err != nil {
			return fmt.Errorf("leyendo p2sodbc.dll base: %w", err)
		}
	}

	dstDll := filepath.Join(appDir, "p2sodbc.dll")
	userStr := append([]byte(user), 0)
	...
	if curData, err := os.ReadFile(dstDll); err == nil {
		if bytes.Contains(curData, passStr) && bytes.Contains(curData, userStr) {
